...
@section('content')
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        ...
        <div class="mb-8 flex gap-4 justify-center flex-wrap">
            ...
            <a href="/" class="px-4 py-2 {{ !request('category') ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-black' }} hover:bg-gray-300 rounded transition">Semua Kategori</a>
            
            <!-- Melakukan iterasi nama Tab Kategori dinamis saat jumlah data bertambah -->
            @foreach($categories as $cat)
                <a href="/?category={{ $cat->slug }}" class="px-4 py-2 {{ request('category') == $cat->slug ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-700' }} hover:bg-indigo-200 rounded shadow-sm transition">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
